...
public class motion_S0506 : MonoBehaviour
{
    ...
    void Update()
    {
        switch(Ball.GetComponent<BallControl_S0506>().s_course)
        {
            case 1:
                if (Ball.GetComponent<BallControl_S0506>().turncou==1)
                {
                    //Bool型のパラメーターであるblRotをTrueにする
                    anim.SetBool("forehand", true);
                }else if (Ball.GetComponent<BallControl_S0506>().turncou==2){
                    //Bool型のパラメーターであるblRotをTrueにする
            ...
            case 4:
                if (Ball.GetComponent<BallControl_S0506>().boundcount==2)
                {
                    //Bool型のパラメーターであるblRotをTrueにする
                    anim.SetBool("frontfore", true);
                }else if (Ball.GetComponent<BallControl_S0506>().boundcount==3||Ball.GetComponent<BallControl_S0506>().boundcount==0){
                    //Bool型のパラメーターであるblRotをTrueにする
