...
public class motion_First : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // pos = new Vector3(x, -56f, 47.5f);

        offset = new Vector3(10.0f, 0.0f, 0.0f);

        x = Ball.transform.position.x + 10.0f;
    }

    // Update is called once per frame
    void Update()
    {
        switch(Ball.GetComponent<BallControl_First>().s_course)
        {
            case 1:
                if (Ball.GetComponent<BallControl_First>().turncou==1)
                {
                    //Bool型のパラメーターであるblRotをTrueにする
                    anim.SetBool("forehand", true);
                }else if (Ball.GetComponent<BallControl_First>().boundcount==2){
                    //Bool型のパラメーターであるblRotをTrueにする
            ...
            case 4:
                if (Ball.GetComponent<BallControl_First>().boundcount==2)
                {
                    //Bool型のパラメーターであるblRotをTrueにする
                    anim.SetBool("frontfore", true);
                }else if (Ball.GetComponent<BallControl_First>().turncou==2){
                    //Bool型のパラメーターであるblRotをTrueにする
